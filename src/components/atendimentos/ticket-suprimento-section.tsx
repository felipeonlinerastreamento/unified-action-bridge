...
import { ShoppingCart, CheckCircle2, Truck, Plus, Trash2 } from "lucide-react";
import {
  useSuprimentoCatalog,
  useTicketSuprimentoItems,
  isSuprimentoCategory,
} from "@/hooks/use-suprimento";
import {
  Select,
  SelectContent,
  SelectItem,
...
export function TicketSuprimentoSection({ ticket, userId, onRefetch }: Props) {
  const qc = useQueryClient();
  const isSup = isSuprimentoCategory(ticket?.category);
  const { data: items = [], refetch } = useTicketSuprimentoItems(isSup ? ticket?.id : null);
  const { data: catalog = [] } = useSuprimentoCatalog(isSup);
  ...
  const setStatus = useMutation({
    mutationFn: async ({ item, status }: { item: any; status: string }) => {
      ...
      await supabase.from("ticket_comments").insert({
        ticket_id: ticket.id,
        user_id: userId,
        content: `Suprimento "${item.quantity}x ${item.item_name}" → ${status}`,
  ...
  const addItem = async () => {
    ...
    setAdding(true);
    try {
      const { error } = await supabase.from("ticket_suprimento_items" as any).insert({
        ticket_id: ticket.id,
        item_id: cat.id,
        item_name: cat.name,
