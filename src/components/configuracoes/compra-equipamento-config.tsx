...
import { Smartphone, Plus, Pencil, Trash2, Loader2 } from "lucide-react";
...
export function CompraEquipamentoConfig() {
  ...
  const { data: items = [], isLoading } = useQuery({
    queryKey: ["compra-equipamento-catalog-config"],
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["compra-equipamento-catalog-config"] });
      qc.invalidateQueries({ queryKey: ["compra-equipamento-catalog"] });
      setDialogOpen(false);
      reset();
    },
      ...
      <CardHeader className="flex flex-row items-start justify-between gap-2">
        <div>
          <CardTitle className="flex items-center gap-2">
            ...
            Itens de Compra (Equipamento/Chip)
          </CardTitle>
          <CardDescription>
        ...
        <DialogContent>
          ...
          <div className="space-y-3">
            <div className="space-y-1">
              <Label>Nome *</Label>
              <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Ex: Chip 4G Vivo / Notebook Dell" />
            </div>
            <div className="space-y-1">
