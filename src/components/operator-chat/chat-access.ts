...
/** Group chat ids where the user still owes a reply (locked participation). */
export async function fetchMyLockedGroupChatIds(userId: string): Promise<Set<string>> {
  const { data } = await supabase
    .from("operator_chat_participants")
    .select("chat_id")
    .eq("user_id", userId)
    .eq("is_locked", true);
  return new Set((data || []).map((r: any) => r.chat_id));
}
